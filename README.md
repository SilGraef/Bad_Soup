...
- Individual annotator sheets, each with these columns: `dataItemId`, `rule_id`, `Final Label`, `Decision`, and `Agreement Count`. `Agreement Count` indicates whether the annotator's `Decision` matches the `Final Label`.
